      ...
      .map((d) => {
        const a = d.data(),
          ...
          fmt = a.submittedAt
            ...
            : "—";
        const sn = (a.studentName || "").replace(/'/g, "\\'"),
          isForwarded = "forwarded_to_admin" === a.status,
          isTerminal = "Admitted" === a.status || "Rejected" === a.status,
          feeStatus = a.admissionFeeStatus || "",
          feeBadge = feeStatus === "verified"
            ? `<span style="background:#d1fae5;color:#065f46;padding:3px 8px;border-radius:12px;font-size:11px;font-weight:600">✅ Verified</span>${a.admissionFee ? `<div style="font-size:11px;color:var(--text-light);margin-top:2px">₹${Number(a.admissionFee).toLocaleString("en-IN")}</div>` : ""}`
            : feeStatus === "failed"
            ...
            ? `<span style="background:#fef3c7;color:#92400e;padding:3px 8px;border-radius:12px;font-size:11px;font-weight:600">⏳ Pending</span>${a.admissionFee ? `<div style="font-size:11px;color:var(--text-light);margin-top:2px">₹${Number(a.admissionFee).toLocaleString("en-IN")}</div>` : ""}`
            : `<span style="font-size:11px;color:var(--text-light)">—</span>`;
        return `<tr>\n          <td><strong>${a.studentName}</strong>${isForwarded ? '<div style="font-size:10px;color:#7c3aed;margin-top:2px"><i class="fas fa-share-square"></i> Forwarded by Office</div>' : ""}</td>\n          <td>${a.class || "—"}</td>\n          <td style="font-size:12px">${a.dob || "—"}</td>\n          <td>${a.parentName || "—"}</td>\n          <td style="font-size:12px">${a.contact || "—"}</td>\n          <td style="font-size:12px">${fmt}</td>\n          <td>${feeBadge}</td>\n          <td><span class="badge ${bc}">${a.status}</span></td>\n          <td>\n            <div style="display:flex;gap:4px;flex-wrap:wrap">\n              <button class="btn btn-sm" style="background:#3b82f6;color:#fff;border:none;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:11px;white-space:nowrap" onclick="adminViewAdmission('${d.id}')"><i class="fas fa-edit"></i> View/Edit</button>\n              <button class="btn btn-sm" style="background:#475569;color:#fff;border:none;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:11px;white-space:nowrap" onclick="adminPrintAdmission('${d.id}')"><i class="fas fa-print"></i> Print</button>\n              ${isForwarded ? `<button class="btn btn-sm" style="background:#059669;color:#fff;border:none;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:11px;white-space:nowrap" onclick="adminApproveAdmission('${d.id}','${sn}')"><i class="fas fa-check-circle"></i> Approve</button><button class="btn btn-sm" style="background:#dc2626;color:#fff;border:none;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:11px;white-space:nowrap" onclick="adminRejectAdmission('${d.id}','${sn}')"><i class="fas fa-times-circle"></i> Reject</button><button class="btn btn-sm" style="background:#d97706;color:#fff;border:none;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:11px;white-space:nowrap" onclick="adminSendBackToOffice('${d.id}','${sn}')"><i class="fas fa-undo"></i> Send Back</button>` : ""}\n              ${!isForwarded && !isTerminal ? `<button class="btn btn-sm" style="background:#1a6b3c;color:#fff;border:none;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:11px;white-space:nowrap" onclick="openApproveModal('${d.id}','${sn}','${a.class || ""}')"><i class="fas fa-check"></i> Approve</button>` : ""}\n              ${"Shortlisted" !== a.status && !isTerminal ? `<button class="btn btn-sm" style="background:#1a4a8a;color:#fff;border:none;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:11px" onclick="updateAdmStatus('${d.id}','Shortlisted')">Shortlist</button>` : ""}\n              ${"Rejected" !== a.status ? `<button class="btn btn-sm btn-danger" style="font-size:11px;padding:4px 8px" onclick="rejectAdmission('${d.id}','${sn}')"><i class="fas fa-times"></i> Reject</button>` : ""}\n            </div>\n          </td>\n        </tr>`;
      })
      .join("")),
  ...
  (window.loadTeacherDashWidgets = async function () {
    ...
    if (holEl)
      try {
        ...
        const calSnap = await getDoc(doc(db, "holidays", "school_calendar"));
        const calData = calSnap.exists() ? calSnap.data() : {};
        const upcoming = [];
        CAL_MONTHS.forEach((month, mi) => {
          const evts = calData[month];
          if (!Array.isArray(evts)) return;
          evts.forEach(ev => {
            if (ev.type !== "holiday" || !ev.date) return;
            const startDay = parseInt(ev.date.toString().split("-")[0], 10);
            if (isNaN(startDay)) return;
            const d = new Date(today.getFullYear(), mi, startDay);
            d.setHours(0, 0, 0, 0);
            if (d >= today) upcoming.push({ date: d, event: ev.event });
          });
        });
        upcoming.sort((a, b) => a.date - b.date);
        upcoming.length
          ? (holEl.innerHTML = upcoming.slice(0, 5)
              .map((h) => {
                const fmt = h.date.toLocaleDateString("en-IN", {
                    weekday: "short",
                    day: "numeric",
                    month: "short",
                    year: "numeric",
                  }),
                  diff = Math.round((h.date - today) / 864e5),
                  tag =
                    0 === diff
  ...
  (window.adminAddFeeRecord = async function () {
    ...
    try {
      (await addDoc(collection(db, "fee_transactions"), {
        ...
        status: status,
        source: window._loginRole === "office" ? "office" : "admin",
        staffName: window._loginRole === "office" ? "Office Staff" : "Admin",
        createdAt: now,
        date: now.split("T")[0],
      }),
  ...
  (async () => {
    ...
    ((window.processPayment = async function () {
      ...
      (window.downloadMonthlyCollectionPDF = async function () {
        ...
        const rows = window._officeReportDocs || [];
        if (!rows.length) return showToast("No data loaded. Apply a date filter and refresh first.");
        // Determine month label from loaded data
        const fromVal = document.getElementById("o-report-from")?.value || "";
        const toVal   = document.getElementById("o-report-to")?.value   || "";
        const monthLabel = fromVal
          ? new Date(fromVal + "T00:00:00").toLocaleDateString("en-IN", { month: "long", year: "numeric" })
          : new Date().toLocaleDateString("en-IN", { month: "long", year: "numeric" });
        const approvedRows = rows.filter(t => t.status === "approved");
        const totalCollected = approvedRows.reduce((s, t) => s + (parseFloat(t.amount) || 0), 0);
        const totalAll = rows.reduce((s, t) => s + (parseFloat(t.amount) || 0), 0);
        ...
        pdf.text("Monthly Fee Collection Report", pw / 2, 31, { align: "center" });
        pdf.setFontSize(9); pdf.setFont("helvetica", "normal");
        pdf.text(`Period: ${monthLabel}   |   Generated: ${new Date().toLocaleString("en-IN")}`, pw / 2, 37, { align: "center" });
        pdf.setDrawColor(139, 111, 71); pdf.setLineWidth(0.5);
        pdf.line(14, 40, pw - 14, 40);
        // Summary bar
        pdf.setFontSize(9); pdf.setFont("helvetica", "bold");
        pdf.text(`Total Transactions: ${rows.length}`, 14, 47);
        pdf.text(`Approved: ${approvedRows.length}`, pw / 2 - 20, 47);
        pdf.text(`Total Collected: Rs.${totalCollected.toLocaleString("en-IN")}`, pw - 14, 47, { align: "right" });
        pdf.line(14, 50, pw - 14, 50);
        // Table
        const bc = s => s === "approved" ? "Approved" : s === "rejected" ? "Rejected" : "Pending";
        pdf.autoTable({
          startY: 54,
          head: [["Date", "Student", "Class", "Amount (Rs.)", "Mode", "Receipt No.", "Staff", "Status"]],
          body: rows.map(t => [
            t.date || "—",
            t.studentName || "—",
            t.studentClass || "—",
          ...
          margin: { left: 14, right: 14 },
          headStyles: { fillColor: [139, 111, 71], textColor: 255, fontStyle: "bold", fontSize: 7.5 },
          ...
          didParseCell(data) {
            if (data.section === "body" && data.column.index === 7) {
              ...
              data.cell.styles.fillColor =
                v === "Approved" ? [209, 236, 214] :
                v === "Rejected" ? [248, 215, 218] : [255, 243, 205];
            }
          },
          didDrawPage() {
            const pg = pdf.internal.getCurrentPageInfo().pageNumber;
        ...
        pdf.setDrawColor(139, 111, 71); pdf.line(14, finalY - 2, pw - 14, finalY - 2);
        pdf.setFontSize(10); pdf.setFont("helvetica", "bold");
        pdf.text(`Total Fee Collected (Approved): Rs.${totalCollected.toLocaleString("en-IN")}`, pw - 14, finalY + 4, { align: "right" });
        pdf.setFontSize(8); pdf.setFont("helvetica", "normal");
        pdf.text(`All transactions total (incl. pending): Rs.${totalAll.toLocaleString("en-IN")}`, pw - 14, finalY + 10, { align: "right" });
        const filename = `monthly-collection-${fromVal || new Date().toISOString().slice(0,7)}.pdf`;
        pdf.save(filename);
