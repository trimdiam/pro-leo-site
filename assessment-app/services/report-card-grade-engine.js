...
export function getTermDateRange(term) {
  ...
  // One-off exception for 2026-27: terms run Feb-Jun / Jul-Nov instead of the
  // usual Apr-Sep / Oct-Mar. Confirmed with school for this academic year only.
  // HY1 opens on 1 February because kindergarten assessment genuinely starts
  // then — SKG has reviewed Literacy and Rhymes & Stories sessions dated
  // 16-27 Feb 2026 that a March start silently dropped from the report card
  // (the aggregator requires a session to be FULLY inside the window). It
  // never reaches into July: that is HY2's window.
  if (getCurrentAcademicYear() === '2026-27') {
    if (term === 'HY1') {
      return {
        dateFrom:  `${startYear}-02-01`,
